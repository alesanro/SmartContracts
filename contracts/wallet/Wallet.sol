...
contract multiowned is WalletEmitter {
    ...
    */
    modifier onlymanyowners() {
        if (isOwner(msg.sender)) {
            uint errorCode = registerTx(this, msg.value, msg.data);
    ...
    *  Virtual function.
    */
    function registerTx(address _to, uint _value, bytes _data) internal returns (uint resultCode);
...
contract Wallet is multiowned {
    ...
    */
    ...
    /**
    *  Transfers funds (ETH or ERC20 tokens) Multi sig operation.
    */
    ...
    /**
    *  Transact entry point.
    *  Goes into multisig process.
    */
    function registerTx(address _to, uint _value, bytes _data)
    internal
    returns (uint resultCode)
    {
        ...
        m_txs[_r].to = _to; // TODO: don's save redundant data
        m_txs[_r].value = _value; // TODO: don's save redundant data
        m_txs[_r].data = _data;
        ...
        if (resultCode != OK) {
            ...
            _emitConfirmationNeeded(_r, msg.sender, _value, _to, _data);
            return WALLET_CONFIRMATION_NEEDED;
        }
        return OK;
    }
    ...
    */
    function confirm(bytes32 _h) public onlyowner returns (uint errorCode) {
       ...
       _emitMultiTransact(msg.sender, _h, m_txs[_h].value, m_txs[_h].to, m_txs[_h].data);
       return OK;
   }
    ...
    /**
    *  Util convert function bytes->bytes32
    */
    function bytes2bytes32(bytes source) public pure returns (bytes32 result) {
      assembly {
